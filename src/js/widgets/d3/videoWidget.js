// New widget
  videoWidget = {
	...
	html: function (id) {
    console.log("WRITING HTML with id:", id)
	...
	video: function (id) {
		...
		function speechDataOptions(chosenDictInfo,
  		...
  		*********************************/
          ...
          if(finalTranscript == undefined){ finalTranscript = ''; };
  				finalTranscript += '<p>' + value.text + '</p>';
  		...
  		transcriptRequest.done(function () {
  			$('#full-transcript').html(finalTranscript);
  		});
      ...
      $('#select-language').change(updateCountry);
		...
		********************************************/
		...
		$.get('php/mongo_transcript_load.php', function (data) {
		 	console.log("Loading from mongo", data);
