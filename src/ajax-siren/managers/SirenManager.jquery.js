 ...
 */
AjaxSolr.Manager = AjaxSolr.SirenAbstractManager.extend(
/** @lends AjaxSolr.Manager.prototype */
{
  ...
  executeRequest : function(servlet, string, handler) {
    ...
    for ( var c in this.classes) {
      if (c != focused && requiredRelations.indexOf(c) != -1) { // add another
                                                                // condition to
                                                                // avoid
                                                                // unnecessary
                                                                // isEmpty if
    ...
    if (this.proxyUrl) {
      ...
      jQuery.getJSON(this.solrUrl + servlet + '?' + string
          + '&wt=json&json.wrf=?', {}, handler);
    }
  ...
  checkForEmptyQuery : function(valueOfQ) {
    if (valueOfQ) {
      if (this._trim(valueOfQ) == "") {
        this.store.removeByValue("q", this.store.get('q').val());
        this.store.addByValue("nested", "{!lucene} *:*");
      } else {
        this.store.removeByValue("nested", "{!lucene} *:*");
      }
      return;
    }

    // here if q param is not present or empty
    if (this.store.get('q').val() == null
        || this._trim(this.store.get('q').val()) == "") {
      this.store.removeByValue("q", this.store.get('q').val());
      this.store.addByValue("nested", "{!lucene} *:*");
    } else {
      this.store.removeByValue("nested", "{!lucene} *:*");
