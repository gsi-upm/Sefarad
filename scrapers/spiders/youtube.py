...
def get_comment_threads(youtube, video_id):
  ...
  for item in results["items"]:
    ...
    result.append({'@type':'Review', 'author':{'@type':'Person', 'name':author}, 'reviewBody':text, 'datePublished':date})
...
if __name__ == "__main__":
  ...
  argparser.add_argument("--path",
    help="Required; Path the result will be stored.")
  ...
  if not args.path:
    exit("Please specify path using the --path= parameter.")

  youtube = get_authenticated_service(args)
  # All the available methods are used in sequence just for the sake of an example.
  try:
    name = args.videoid
    image = "http://img.youtube.com/vi/%s/maxresdefault.jpg" % args.videoid
    video_comment_threads = get_comment_threads(youtube, args.videoid)
    result = {'@context':'http://schema.org', 'name':name, 'image':image, 'reviews':video_comment_threads}
    with open(args.path, 'w') as file:
      file.write(json.dumps(result))
    #print json.dumps(video_comment_threads)
