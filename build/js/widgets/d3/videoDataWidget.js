...
var videoDataWidget = {
  ...
  paint: function (id) {
    'use strict';
    console.log("START: PAINTING GRAPHIC with id:", id)
    $('#' + id).html('<div id="line-graph-box"><h2 class="title" id="line-graph-title">Phrase vs. Sentiment Progression</h2><p class="sentiment" id="hover-text" ></p><p class="phrase" id="clicked-phrase" style="text-align: center; padding-left: 5%; display: inline-block;"></p><svg class="graph" id="line-graph"></svg></div>')
    $('#' + id).append('<style>#line-graph-box{background:white;}#line-graph-box .graph{height:100%;width:100%}#line-graph-box .graph .axis line,#line-graph-box .graph .axis path{fill:none;stroke:black;shape-rendering:crispEdges}#line-graph-box .graph .line{fill:none;stroke:#0b6693;stroke-width:2px}#line-graph-box .phrase{color:#053349;display:none;font-size:1em;margin-top:1%}#line-graph-box .sentiment{color:#0b6693;display:none;font-size:1.1em;margin:0 auto;padding-left:5%;position:relative;text-align:right;top:4px}#line-graph-box .title{color:#053349;display:inline;font-size:1.5em;padding-left:11%;position:relative;text-align:left;top:4px}</style>');
    ...
    ***************************************/
    updateLineGraph = function updateLineGraph(options) {
      console.log("START: updateLineGraph")
      ...
      d3.select(this).attr('r', 7)
        .moveToFront();
      var cy = Math.round(1000 * y.invert(d3.select(this).attr('cy'))) / 1000;
      d3.select('#hover-text').text(function () { return 'Sentiment: '+cy; })
        .style('display', 'inline');
      };
