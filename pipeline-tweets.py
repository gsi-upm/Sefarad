...
class FetchDataTask(luigi.Task):
	...
	with open(file) as f:
                j = json.load(f)
		for i in j:
			i["_id"] = str(random.randint(0,10000))
	...
	with self.output().open('w') as output:
		with self.input().open('r') as infile:
			...
			if(self.analysis == 'emotions'):
				for i in j:
					r = requests.get('http://senpy.cluster.gsi.dit.upm.es/api/?algo=EmoTextANEW&i=%s' % i["text"])
					response = r.content.decode('utf-8')
					response_json = json.loads(response)
					i["analysis"] = response_json
					i["emotion"] = response_json["entries"][0]["sentiments"][0]["marl:hasPolarity"]	
					i["polarity"] = response_json["entries"][0]["sentiments"][0]["marl:polarityValue"]   
					output.write(json.dumps(i))
					output.write('\n')
